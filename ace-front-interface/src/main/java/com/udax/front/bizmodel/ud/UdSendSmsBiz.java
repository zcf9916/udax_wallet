package com.udax.front.bizmodel.ud;
 ...
 * @version 1.0
 */
public abstract class UdSendSmsBiz {
    ...
    //通过短信类型校验各种情况
    protected ObjectRestResponse validSmsType(SmsModel model, HUserInfoBiz biz){
    	 //注册
         if(model.getSendMsgType().equals(SendMsgType.USER_REG.value())) {
         	 Map<String,Object> params = new HashMap<String,Object>();
         	 params.put("userName",model.getUserName());
			 FrontUser user = ServiceUtil.selectUDUnionUserInfoByUserName(model.getUserName(),biz);
			 if( user != null && user.getHUserInfo() != null && user.getHUserInfo().getUserId() != null){
...
//		 }
	...
	//发送验证码
	public abstract ObjectRestResponse sendValidCode(SmsModel model, HUserInfoBiz userBiz, Long exchId);
