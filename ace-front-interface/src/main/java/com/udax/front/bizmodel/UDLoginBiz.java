...
public class UDLoginBiz {
    ...
    private HUserInfoBiz biz;


    public UDLoginBiz(LoginModel loginModel, HUserInfoBiz biz,  JwtTokenUtil util) {
    ...
    public ObjectRestResponse login(HttpServletRequest  request){
        try{
            ...
            UDUserInfoRspVo rspVo = new UDUserInfoRspVo();
            BeanUtils.copyProperties(frontUser,rspVo);
            BeanUtils.copyProperties(frontUser.getHUserInfo(),rspVo);
...
//            }
    ...
    private String dealToken(HttpServletRequest request) throws Exception {
        String token = jwtTokenUtil.generateToken(new JWTInfo(frontUser.getUserName(), frontUser.getId(), frontUser.getUid(),frontUser.getHUserInfo().getExchangeId()));
...
//        }
    ...
    private ObjectRestResponse validateQueryInfo(){
        //通过用户名查询出用户
        frontUser = ServiceUtil.selectUDUnionUserInfoByUserName(loginModel.getUsername(),biz);
        //FrontUser frontUser = null;
        //如果查不出用户
        if (frontUser == null || frontUser.getHUserInfo() == null || frontUser.getHUserInfo().getUserId() == null) {
            return new ObjectRestResponse().status(ResponseCode.ACCOUNT_PWD_ERROR);
        }
