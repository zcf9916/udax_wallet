...
public class HOrderDetailBiz extends BaseBiz<HOrderDetailMapper, HOrderDetail> {
    ...
    @Autowired
    HNodeAwardMapper nodeAwardMapper;

    @Autowired
    private HPurchaseLevelBiz purchaseLevelBiz;

    @Transactional(rollbackFor = Exception.class)
    public void adminSettleProfit(String orderNo)   throws Exception{

            logger.error("管理后台手动开始结算订单分成,订单号:" + orderNo);

            HSettledProfit profit = settledProfitMapper.selectOrderNoForUpdate(orderNo);//锁定当前订单

            if(profit == null){
                throw new BusinessException(ResponseCode.ORDER_NOT_EXIST.name());
            }

            if(!SettledProfitStatus.WAIT_SETTLE.value().equals(profit.getStatus())){
                throw new BusinessException(ResponseCode.NOT_SETTLE_CONDITION.name());
            }
            ...
            dcAssertAccountBiz.dcSignUpdateAssert(payVo,AccountSignType.ACCOUNT_WITHDRAW_DEDUTION);
            ...
            dcAssertAccountBiz.dcSignUpdateAssert(receive,AccountSignType.ACCOUNT_RECHARGE);
            ...
            example.createCriteria().andEqualTo("userId",profit.getUserId());
    ...
    private  void userProfit(HSettledProfit profit,Long exchId) throws Exception{
        ...
        if(StringUtil.listIsNotBlank(relationsList)){
...
//                HPurchaseLevel receivelevel = purchaseLevelBiz.getProximalLevel(userInfo.getTotalAmount().add(userInfo.getAddAmount()));
//                HPurchaseLevel currentLevel = purchaseLevelBiz.selectById(profit.getLevelId());
    ...
    private BigDecimal platCharge(HSettledProfit profit,Long exchId) throws Exception{
        ...
        if(profitRate.compareTo(BigDecimal.ZERO) > 0){
            ...
            dcAssertAccountBiz.dcSignUpdateAssert(chargeVo,AccountSignType.ACCOUNT_PAY_AVAILABLE);
        }
            //生成平台的手续费收益数据
            HCommissionDetail detail  = new HCommissionDetail();
